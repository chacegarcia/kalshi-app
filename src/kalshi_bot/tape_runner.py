    ...
    *,
    ...
    if settings.trade_entry_cap_long_yes_max > 0 and (settings.trade_entry_cap_long_yes_substring or "").strip():
        try:
            ...
            log.warning("tape_portfolio_snapshot_fail", error=str(exc))
            snap_for_cap = None
    ...
    for i, (ticker, flow_usd, n_trades) in enumerate(top):
        ...
        try:
            ...
            if should_skip_buy_ticker_substrings(settings, ticker):
                ...
                log.info("tape_skip_ticker_substring", ticker=ticker)
                continue
